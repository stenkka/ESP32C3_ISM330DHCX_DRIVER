...
#ifdef CONFIG_SENSORS_ISM330DHCX
 ...
 */
   ...
   * capabilities.
   */
#if defined(CONFIG_SENSORS_ISM330DHCX) && defined(CONFIG_ESP32C3_SPI2)
struct spi_dev_s *ism330dhcx_spi = esp32c3_spibus_initialize(ESP32C3_SPI2);
if(!ism330dhcx_spi) {
	syslog(LOG_ERR, "Failed to initialize SPI%d port\n", ESP32C3_SPI2);
}
ret = ism330dhcx_register("/dev/ism330dhcx", ism330dhcx_spi);
if (ret < 0) {
	syslog(LOG_ERR, "Failed to initialize /dev/ism330dhcx driver\n");
}
// if registering the driver was successful, we move on with the initialization
else
{
